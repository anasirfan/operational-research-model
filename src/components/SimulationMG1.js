...
const SimulationMG1 = () => {
  ...
  useEffect(() => {
    ...
    if (!isNaN(arrivalMeanParam) && !isNaN(serviceMeanParam) && selectedDistribution) {
      ...
      const calculatedData = calculateCalculatedData(data, serviceDistribution);
  ...
  const calculateCalculatedData = (data, serviceDistribution) => {
    ...
    if (serviceDistribution === 'gamma') {
      ...
      serverUtilization = serverUtilizationTime / (serverUtilizationTime + serverIdleTime);
      serverIdle = serverIdleTime / (serverUtilizationTime + serverIdleTime);
    } else if (serviceDistribution === 'normal') {
      // Calculate server utilization and idle time for normal distribution
      const normalStandardDeviation = 1; // Example standard deviation for normal distribution
    ...
    return {
      calculatedData,
      serverIdleTimeData,
      serverUtilizationTimeData,
      averageWaitTime,
      averageTurnaroundTime,
      averageServerIdleTime,
      serverUtilization,
      serverIdle,
      expectedServiceTime,
    };
