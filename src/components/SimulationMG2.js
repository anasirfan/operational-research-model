...
const SimulationMG2 = () => {
    ...
    useEffect(() => {
      ...
      if (!isNaN(arrivalMeanParam) && !isNaN(serviceMeanParam) && selectedDistribution) {
          ...
          const calculatedData = calculateCalculatedData(data, serviceDistribution, serviceMean);
  ...
  const calculateCalculatedData = (data, serviceDistribution, serviceMean) => {
    ...
    const totalIdleTime = totalServer1IdleTime + totalServer2IdleTime;
    const totalUtilizationTime =
      totalServer1UtilizationTime + totalServer2UtilizationTime;
    const totalSystemTime = Math.max(totalServer1Time, totalServer2Time);
    const totalSystemIdleTime = totalSystemTime - totalUtilizationTime;
    
    const server1IdlePercentage = Math.abs(((totalServer1IdleTime / totalServer1Time) * 100).toFixed(2));
    const server2IdlePercentage = Math.abs(((totalServer2IdleTime / totalServer2Time) * 100).toFixed(2));
    const systemIdlePercentage = Math.abs(((totalSystemIdleTime / totalSystemTime) * 100).toFixed(2));
    
    const server1UtilizationPercentage = ((totalServer1UtilizationTime / totalServer1Time) * 100).toFixed(2);
    const server2UtilizationPercentage = ((totalServer2UtilizationTime / totalServer2Time) * 100).toFixed(2);
    const systemUtilizationPercentage = ((totalUtilizationTime / totalSystemTime) * 100).toFixed(2);
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div className="w-full max-w-3xl flex justify-center space-x-4 my-8">
        <button
          className={`tab-button bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${activeTab === 'random' ? 'active' : ''}`}
          onClick={() => handleTabChange('random')}
        >
          Random Data
        </button>
        <button
          className={`tab-button bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${activeTab === 'calculated' ? 'active' : ''}`}
          onClick={() => handleTabChange('calculated')}
        >
          Calculated Data
        </button>
        <button
          className={`tab-button  bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${activeTab === 'graphical' ? 'active' : ''}`}
      ...
      {activeTab === 'random' && (
    <>
      <p className="text-center text-lg underline">Using Distribution: {serviceDistribution}</p>
